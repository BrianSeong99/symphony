defmodule SymphonyElixir.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "github", tracker_repository: nil)
  ...
  test "prompt builder can render compact runtime prompts for fast runner execution" do
    ...
    assert prompt =~ "Treat injected `AGENTS.md`/`CLAUDE.md` guidance as already loaded"
    assert prompt =~ "Start with `git diff --stat`"
    assert prompt =~ "Keep shell commands single-purpose"
    ...
    refute prompt =~ "Read nearest `AGENTS.md`"
