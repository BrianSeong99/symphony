...
tracker:
  kind: github
  repository: TheChainlessLabs/omega-zone
  active_labels:
    - agent:symphony
  terminal_states:
    - Closed
...
hooks:
  after_create: |
    if git remote get-url origin >/dev/null 2>&1; then
      git remote set-url origin git@github.com:TheChainlessLabs/omega-zone.git
    else
      git remote add origin git@github.com:TheChainlessLabs/omega-zone.git
    fi
...
You are working on a GitHub issue in the Omega Zone repository.
...
This is a public Chainless/Omega repo. Treat public GitHub artifacts as
public-safe only: no private Symphony workpads, private planning context, hidden
dependency graphs, internal strategy, or sensitive operational notes. Respect
the team's existing labels, milestones, projects, branch protection, issue
templates, and release conventions unless the issue explicitly asks for a
workflow change.
