...
polling:
  interval_ms: 10000
workspace:
  root: ~/Develop/Labs/worktrees/symphony-cmo-github-test
  source_repo: ~/Develop/Labs/Marketing-OS/cmo
...
hooks:
  after_create: |
    if git remote get-url origin >/dev/null 2>&1; then
      git remote set-url origin git@github.com:BrianSeong99/cmo.git
    else
      git remote add origin git@github.com:BrianSeong99/cmo.git
    fi
...
agent:
  ...
  prompt_mode: compact
codex:
  command: SYMPHONY_GIT_BASE_REF=origin/main SYMPHONY_GIT_PUSH_REMOTE=origin SYMPHONY_GITHUB_REPO=BrianSeong99/cmo SYMPHONY_GITHUB_BASE=main SYMPHONY_RUNNER_ENABLED=false SYMPHONY_SERVER_PORT=0 codex --dangerously-bypass-approvals-and-sandbox --config shell_environment_policy.inherit=all --config 'model="gpt-5.5"' --config model_reasoning_effort=xhigh app-server
...
You are working on a GitHub issue in Brian's CMO / Marketing-OS repository.
...
blocks. Optional connectors and MCP tools are not required for CMO GitHub issue
work.

Finish the issue end-to-end: implement, validate, commit, push, open a PR that
closes the issue, and self-merge when checks pass and repository policy allows
it.
